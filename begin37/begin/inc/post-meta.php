		...
		if (zm_get_option('meta_author')) {
			...
			if (zm_get_option('wow_no')) {
				...
				if (zm_get_option('cache_avatar')) {
					echo begin_avatar( get_the_author_meta('email'), '64', '', get_the_author() );
				} else {
					echo get_avatar( get_the_author_meta('email'), '64', '', get_the_author() );
				}
				echo '</span>';
				author_inf();
			}
...
function begin_page_meta_zt() {
	...
	echo '<div class="single-meta-zt begin-single-meta">';
		...
		echo '<span><i class="be be-paper"></i> ';
