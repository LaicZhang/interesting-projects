...
<div id="fep-new-post">
	...
	<form id="fep-submission-form">
		...
		<input type="text" name="post_title" id="fep-post-title" class="bk" value="<?php echo ($post) ? $post['title'] : ''; ?>">
		...
		wp_editor($post['content'], 'fep-post-content', $settings = array('textarea_name' => 'post_content', 'editor_class' => 'toug', 'textarea_rows' => 7, 'media_buttons' => $enable_media));
		...
		<?php 
			...
			wp_dropdown_categories(array('id' => 'fep-category', 'class' => 'bk', 'hide_empty' => 0, 'name' => 'post_category', 'orderby' => 'name', 'selected' => $post['category'], 'hierarchical' => true,  'exclude' => $notcat));
		?>
		<label for="fep-tags">标签</label>
		<input type="text" name="post_tags" id="fep-tags" class="bk" value="<?php echo ($post) ? $post['tags'] : ''; ?>">
