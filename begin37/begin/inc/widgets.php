...
<?php if (zm_get_option('cat_icon') && $instance['show_icon']) { ?>
	...
	<h3 class="widget-title-icon cat-w-icon da"><a href="<?php echo $titleUrl; ?>" rel="bookmark"><i class="t-icon <?php echo zm_taxonomy_icon_code(); ?>"></i><?php echo $instance['title_z']; ?><?php more_i(); ?></a></h3>
<?php } ?>
		    ...
		    foreach ($counts as $count) {
		      $c_url = $count->comment_author_url;
				if (zm_get_option('cache_avatar')) {
			      $mostactive .= '<div class="readers-avatar"><span>' . '<a href="'. get_template_directory_uri()."/go.php?url=".base64_encode($c_url) . '" title="' . $count->comment_author .'  '. $count->cnt . ' 个脚印" target="_blank" rel="external nofollow">' . begin_avatar($count->comment_author_email, 96, '', $count->comment_author) . '</a></span></div>';
				} else {
			      $mostactive .= '<div class="readers-avatar"><span>' . '<a href="'. get_template_directory_uri()."/go.php?url=".base64_encode($c_url) . '" title="' . $count->comment_author .'  '. $count->cnt . ' 个脚印" target="_blank" rel="external nofollow">' . get_avatar($count->comment_author_email, 96, '', $count->comment_author) . '</a></span></div>';
				}
		    }
		  echo $mostactive;
		  }
...
<div id="feed_widget">
	<div class="feed-rss">
		<ul>
			<li class="weixin">
				<span class="weixin-b"><span class="weixin-qr bk"><span class="weixin-arrow"><span class="arrow-s"><i class="be be-favorite"></i></span><span class="arrow-b"><i class="be be-favorite"></i></span></span><img src="<?php echo $instance['weixin']; ?>" alt=" weixin"/></span><a title="微信"><i class="be be-weixin"></i></a></span>
			</li>
...
<?php if($instance['show_icon']) { ?>
	<h3 class="widget-title-cat-icon cat-w-icon da"><a href="<?php echo $titleUrl; ?>" rel="bookmark"><i class="t-icon <?php echo $instance['show_icon']; ?>"></i><?php echo $instance['title_z']; ?><?php more_i(); ?></a></h3>
<?php } ?>
		...
		if ($newWindow) $newWindow = "target='_blank'";
			...
			if($instance['show_icon']) {
				if($instance['titleUrl']) {
					echo '<h3 class="widget-title bkx da"><a href=' . $titleUrl . '><i class="t-icon ' . $instance['show_icon'] . '"></i>' . $instance['title_z'], more_i() . '</a></h3>';
				} else {
					echo '<h3 class="widget-title bkx da"><i class="t-icon ' . $instance['show_icon'] . '"></i>' . $instance['title_z'] . '</h3>';
				}
			}
...
<div class="author_widget_box">
	<?php
		foreach ( $comment_counts as $count ) {
			...
			if (zm_get_option('cache_avatar')) {
				$postount = begin_avatar( $user->user_email, $size = 96, '', $user->display_name );
			} else {
				$postount = get_avatar( $user->user_email, $size = 96, '', $user->display_name );
			}
...
class user_login extends WP_Widget {
	...
	static function login_form() {
		...
		if( !$user_ID || '' == $user_ID ) {
			...
			$html .= '<input type="text" name="login_username" class="bk da" placeholder="'.sprintf(__( '用户名', 'begin' )).'" /><br/>';
			$html .= '<input type="password" name="login_password" class="bk da" placeholder="'.sprintf(__( '密码', 'begin' )).'" /><br/>';
			$html .= '<input type="submit" class="bk da" name="login_submit" value="'.sprintf(__( '登录', 'begin' )).'" /><br/>';
	...
	static function registration_form() {
		...
		if( !$user_ID || '' == $user_ID ) {
			$html = '<div id="login_widget"><form method="post" action="' . esc_url( $_SERVER['REQUEST_URI'] ) . '">';
			$html .= '<input type="text" name="registration_username" class="bk da" placeholder="用户名" /><br/>';
			$html .= '<input type="password" name="registration_password" class="bk da" placeholder="密码" /><br/>';
			$html .= '<input type="email" name="registration_email" class="bk da" placeholder="邮件" /><br/>';
			$html .= '<input type="submit" class="bk da" name="reg_submit" value="注册" /><br/>';
			$html .= '</form></div>';
			return $html;
		}
	}
	...
	public function widget( $args, $instance ) { ?>
		...
		<div class="login-tab-widget">
			<div class="tab-title">
				<h3>
					<span class="selected"><?php _e( '登录', 'begin' ); ?></span>
					<?php if (!is_user_logged_in() && get_option('users_can_register')) { ?>
						<span><?php _e( '注册', 'begin' ); ?></span>
					<?php } ?>
				</h3>
			</div>
			<div class="login-tab-content">
				...
				<?php if (!is_user_logged_in() && get_option('users_can_register')) { ?>
					<ul class="tab-reg">
						<?php if($instance['custom_register']) { ?>
							<li><a class="widget-register-url bk" href="<?php echo $instance['register_url']; ?>" target="_blank"><?php _e( '点此注册', 'begin' ); ?></a></li>
						<?php } else { ?>
							<li><?php echo self::registration_form(); ?>
						<?php } ?>
					</ul>
				<?php } ?>
							...
							<div class="login-widget-avata">
								<?php 
									...
									if (zm_get_option('cache_avatar')) {
										...
										echo get_avatar( $userdata->user_email, 96, '', $user_identity );
									}
								?>
								<div class="clear"></div>
								<?php _e( '您已登录', 'begin' ); ?>：<?php echo $user_identity; ?>
							</div>
	...
	public function form( $instance ) {
		...
		<p>
			<input type="checkbox" class="checkbox" id="<?php echo esc_attr( $this->get_field_id('custom_register') ); ?>" name="<?php echo esc_attr( $this->get_field_name('custom_register') ); ?>" <?php checked( (bool) $instance["custom_register"], true ); ?>>
			<label for="<?php echo esc_attr( $this->get_field_id('custom_register') ); ?>">自定义注册链接</label>
		</p>
	<p>
		<label for="<?php echo $this->get_field_id('register_url'); ?>">注册链接地址：</label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'register_url' ); ?>" name="<?php echo $this->get_field_name( 'register_url' ); ?>" type="text" value="<?php echo $register_url; ?>" />
	</p>
	<?php
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['custom_register'] = $new_instance['custom_register']?1:0;
		$instance['register_url'] = $new_instance['register_url'];
		return $instance;
	}
...
<div class="w-img-cat">
	<?php if (zm_get_option('cat_icon') && $instance['show_icon']) { ?>
		<?php $q = '&category__and='.$instance['cat']; query_posts($q);?>
		<h3 class="widget-title-cat-icon cat-w-icon da"><a href="<?php echo $titleUrl; ?>" rel="bookmark"><i class="t-icon <?php echo zm_taxonomy_icon_code(); ?>"></i><?php echo $instance['title_z']; ?><?php more_i(); ?></a></h3>
	<?php } ?>
		<?php 
			global $post;
			if ( is_single() ) {
...
<div class="widget-cat-cover">
	...
	<?php if($instance['show_tags']) { ?>
		...
		?>
		<div class="cover4x">
			<div class="cat-cover-main wow fadeInUp da">
