...
function article_catalog($content) {
	...
	if(preg_match_all($r, $content, $matches)) {
		...
		$content = "
			\n<div id=\"log-box\" class=\"bk da\">
				<div id=\"catalog\"><ul id=\"catalog-ul\">\n" . $ul_li . "</ul><span class=\"log-zd bk da\"><span class=\"log-close\"><a title=\"" . sprintf(__( '隐藏目录', 'begin' )) . "\"><i class=\"be be-cross\"></i><strong>" . sprintf(__( '目录', 'begin' )) . "</strong></a></span></span></div>
			</div>\n" . $content;
	}
	return $content;
}
...
function get_author_admin($comment_author_email,$user_id){
	...
	$author_count = count($wpdb->get_results(
	"SELECT comment_ID as author_count FROM $wpdb->comments WHERE comment_author_email = '$comment_author_email' "));
	$adminEmail = get_option('admin_email');if($comment_author_email ==$adminEmail) echo '<span class="author-admin"><i class="be be-accountbox"></i></span>';
}
...
function remove_menus(){
	...
	remove_menu_page( 'edit.php?post_type=surl' );// 短链接
}
