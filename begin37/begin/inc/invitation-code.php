...
if ( ! is_admin() ) {
	...
	?>
		<p>
			...
			<?php if ( ! empty( $begin_fields['link'] ) && $begin_fields['link'] == 'on' ) { ?>
				<div class="to-code"><a href="<?php echo ! empty( $begin_fields['text_link'] ) ? wp_kses_post( $begin_fields['text_link'], $allowedposttags ) : ''; ?>" target="_blank"><?php _e( '获取邀请码', 'begin' ); ?></a></div>
			<?php } ?>
		</p>
	 <?php
	}
	...
	function begin_field_link() {
		$begin_fields = get_option( 'begin_fields' );
	?>
		<label><input type="checkbox" class="checkbox" name="begin_fields[link]" <?php checked( $begin_fields['link'], 'on' ); ?>/> 在注册表单添加获取邀请码链接。</label>
	<?php
	}
	...
	function begin_admin_menu() {
		...
		add_submenu_page( 'begin_list_codes', '全邀请码', '全部邀请码', 'manage_options', 'begin_list_codes', 'begin_list_codes' );
	...
	function begin_codes_settings_page() {
		...
		add_settings_field( 'begin_field_code', __( '邀请码链接', 'begin' ), 'begin_field_link', 'begin_codes_settings', 'begin_codes_settings' );
	...
	?>
		<div class="wrap">
			...
			<h4>前端邀请码使用状态</h4>
			<p>编辑文章或者页面添加短代码：<code>[invite]</code>，并将链接地址填写到上面链接地址中。</p>
		</div>
	<?php
	}
						...
						<tr class="token">
							<td>
								<div class="activation">
									<pre><?php echo esc_html( $code ); ?></pre>
								</div>
							</td>
							<td>
								<div class="activation">
									<?php echo '尚可使用 ' . $infos['leftcount'] . ' 次，共 ' . (int) $infos['maxcount']; ?>
								</div>
							</td>
...
function begin_invite_list() {
	$begin_codes_options = get_option( 'begin_codes_options' );
?>
<table id="codes-table" class="codes-list">
	<thead>
		<tr>
			<th scope="col" width="350"><?php _e( '邀请码', 'begin' ); ?></th>
			<th scope="col" width="350"><?php _e( '状态', 'begin' ); ?></th>
		</tr>
	</thead>
	<tbody class="codes_table">
		<?php
		...
		?>
			<tr class="token">
				...
				<td>
					<?php echo '尚可使用 ' . $infos['leftcount'] . ' 次'; ?>
				</td>
			</tr>
		<?php } ?>
	</tbody>
...
function begin_invite_codes() {
	begin_invite_list();
}
add_shortcode('invite', 'begin_invite_codes');