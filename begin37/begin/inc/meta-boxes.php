...
function thumbnail_post_meta_boxes() {
	...
	foreach ($thumbnail_post_meta_boxes as $meta_box) {
		...
		switch ($meta_box['type']) {
			...
			case 'upload':
				echo '<h4>' . $meta_box['title'] . '</h4>';
				echo '<span class="form-field"><input type="text" size="40" name="' . $meta_box['name'] . '" value="' . $meta_box['std'] . '" /><a href="javascript:;" class="begin_file button">选择图片</a></span><br />';
			break;
			}
		}
}
...
function header_show_meta_boxes() {
	...
	foreach ($header_show_meta_boxes as $meta_box) {
		...
		switch ($meta_box['type']) {
			...
			case 'textarea':
				echo '<h4>' . $meta_box['title'] . '</h4>';
				echo '<textarea id="seo-excerpt" cols="40" rows="2" name="' . $meta_box['name'] . '">' . $meta_box['std'] . '</textarea><a href="javascript:;" class="begin_file button">选择图片</a><br />';
			break;
			}
		}
}
