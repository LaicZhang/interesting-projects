		...
		<div id="respond" class="comment-respond wow fadeInUp ms bk da">
			...
			<?php else : ?>
				<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
							...
							<div class="user-avatar">
								<?php global $current_user;wp_get_current_user();
									if (zm_get_option('cache_avatar')) {
										echo begin_avatar( $current_user->user_email, 64, '', $user_identity );
									} else {
										echo get_avatar( $current_user->user_email, 64, '', $user_identity );
									}
								?>
							</div>
							<div class="comment-user-inc">
						...
						<div class="comment-user-inf">
							<div class="user-avatar">
								<?php if (zm_get_option('cache_avatar')) { ?>
									<?php echo begin_avatar($comment_author_email, $size = '64', '', $comment_author );  ?>
								<?php } else { ?>
									<?php echo get_avatar($comment_author_email, $size = '64', '', $comment_author );  ?>
								<?php } ?>
							</div>
							<div class="comment-user-inc">
								<h3 id="reply-title" class="comment-reply-title">
					...
					<?php if (zm_get_option('emoji_show')) { ?><p class="emoji-box"><?php get_template_part( 'inc/smiley' ); ?></p><?php } ?>
					...
					<?php } ?>
						<p class="comment-form-author">
							...
							<input type="text" name="author" id="author" class="commenttext da" value="<?php echo $comment_author; ?>" tabindex="2" />
						</p>
						<?php if ( zm_get_option('no_email') == '' ) { ?>
							<p class="comment-form-email">
								...
								<input type="text" name="email" id="email" class="commenttext da" value="<?php echo $comment_author_email; ?>" tabindex="3" />
							</p>
