					...
					<div id="tab1_login" class="tab_content_login">
						<form id="login-box" class="wp-user-form" action="<?php echo get_option('home'); ?>/wp-login.php" method="post" novalidate="novalidate">
							...
							<p class="login_fields">
								<input class="user-submit da bk" type="submit" name="user-submit" value="<?php _e( '登录', 'begin' ); ?>" />
				...
				<?php } else { ?>
							...
							<?php if (zm_get_option('invitation_code') || zm_get_option('go_reg') ) { ?>
								<form method="post" action="<?php echo site_url('wp-login.php?action=register', 'login_post') ?>" class="wp-user-form">
									<div class="username">
										...
										<input type="text" name="user_login" value="<?php echo esc_attr(stripslashes($user_login)); ?>" size="20" id="user_reg" class="da" required="required" />
									</div>
									<div class="password">
										<label for="user_email"><?php _e( '电子邮件地址', 'begin' ); ?></label>
										<input type="text" name="user_email" value="<?php echo esc_attr(stripslashes($user_email)); ?>" size="25" id="user_email" class="da" required="required" />
									</div>
							...
							<?php } else { ?>
								<form id="register" class="wp-user-form" action="<?php bloginfo('url'); ?>/wp-login.php?action=register" method="post" novalidate="novalidate">
									...
									<p>
										...
										<input class="input-control" id="user_pass" class="da" type="password" name="user_pass" placeholder="" required="" aria-required="true">
									</p>
									<p>
										<label for="user_email"><?php _e( '重复密码', 'begin' ); ?></label>
										<input class="input-control" type="password" id="user_pass2" class="da" name="user_pass2" placeholder="" required="" aria-required="true">
									</p>
					...
					<?php } else { ?>
						<div class="login-tab-bd-con reg-url">
							<?php if ( zm_get_option('reg_url') == '' ) { ?><?php } else { ?><a class="bk" href="<?php echo stripslashes( zm_get_option('reg_url') ); ?>" target="_blank"><?php _e( '立即注册', 'begin' ); ?></a><?php } ?>
		...
		<div class="sidebox bk">
			...
			<div class="userinfo">
				<p>
					...
					echo '<a class="bk da" href="' . admin_url() . '" target="_blank">'.sprintf(__( '管理站点', 'begin' )).'</a>'; } else { 
					echo '<a class="bk da" href="' .get_permalink( zm_get_option('user_url') ) . '" target="_blank">'.sprintf(__( '用户中心', 'begin' )).'</a>'; } ?>
					<a class="bk da" href="<?php echo wp_logout_url('index.php'); ?>"><?php _e( '登出', 'begin' ); ?></a>
				</p>
				<div class="clear"></div>
			</div>
		</div>
