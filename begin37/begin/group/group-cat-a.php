					...
					<div class="gr-cat-site">
						<?php if (zm_get_option('group_cat_a_top')) { ?>
							...
							<?php query_posts( array ( 'category__in' => array(get_query_var('cat')), 'meta_key' => 'cat_top', 'showposts' => 1, 'ignore_sticky_posts' => 1 ) ); while ( have_posts() ) : the_post(); $do_not_cat[] = $post->ID;?>
								<div id="post-<?php the_ID(); ?>" class="gr-img-t wow fadeInUp">
									<figure class="gr-thumbnail"><?php zm_long_thumbnail(); ?></figure>
									<?php the_title( sprintf( '<h2 class="gr-title-img wow fadeInUp bgb"><a href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h2>' ); ?>
									<div class="clear"></div>
								</div>
							<?php endwhile; ?>
							...
							<div class="gr-cat-img">
								...
								<div class="cat-gr2">
									<div id="post-<?php the_ID(); ?>" class="gr-img wow fadeInUp">
										<figure class="gr-a-thumbnail picture-cms-img"><?php zm_thumbnail(); ?></figure>
										<?php the_title( sprintf( '<div class="gr-img-title wow fadeIn"><a class="bgt hz" href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></div>' ); ?>
										<div class="clear"></div>
									</div>
								</div>
								<?php endwhile; ?>
						...
						<?php } else { ?>
							...
							<?php while ( have_posts() ) : the_post(); ?>
								<div id="post-<?php the_ID(); ?>" class="gr-img-t wow fadeInUp">
									<figure class="gr-thumbnail"><?php zm_long_thumbnail(); ?></figure>
									<?php the_title( sprintf( '<h2 class="gr-title-img wow fadeInUp bgb"><a class="bgt hz" href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h2>' ); ?>
									<div class="clear"></div>
								</div>
							<?php endwhile; ?>
							...
							<div class="gr-cat-img">
								...
								<?php while ( have_posts() ) : the_post(); ?>
								<div id="post-<?php the_ID(); ?>" class="cat-gr2 wow fadeInUp">
									<div class="gr-img">
										<figure class="gr-a-thumbnail picture-cms-img"><?php zm_thumbnail(); ?></figure>
										<?php the_title( sprintf( '<div class="gr-img-title wow fadeIn"><a class="bgt hz" href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></div>' ); ?>
										<div class="clear"></div>
