	...
	<div class="g-col">
		<div class="deanm">
			...
			<div class="deanm-main wow fadeInUp">
				<?php $posts = get_posts( array( 'post_type' => 'any', 'orderby' => 'menu_order', 'meta_key' => 'pr_a', 'numberposts' => '16') ); if($posts) : foreach( $posts as $post ) : setup_postdata( $post ); ?>
					<?php 
					...
					?>
				<div class="deanm sup deanmove da bk">
					<?php the_title( sprintf( '<div class="de-t wow fadeIn"><a href="%s" rel="bookmark">', esc_url( $pr_d ) ), '</a></div>' ); ?>
					<div class="clear"></div>
					<div class="de-a wow fadeIn"><?php echo $pr_b; ?></div>
					<div class="deanquan sup">
						<a href="<?php echo $pr_d; ?>" target="_blank">
							<div class="de-back">
								...
								<div class="de-b wow fadeIn bgt"><?php echo $pr_a; ?></div>
							</div>
						</a>
					</div>
					<div class="de-c wow fadeIn"><?php echo $pr_c; ?></div>
					<div class="de-button wow fadeIn">
						<a class="da" href="<?php echo $pr_d; ?>" target="_blank"><?php if ( get_post_meta($post->ID, 'pr_e', true) ) { ?><?php echo $pr_e; ?><?php } else { ?><i class="be be-stack"></i>按钮名称<?php } ?></a>
					</div>
				</div>
