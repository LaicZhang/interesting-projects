...
<div class="g-row <?php if (zm_get_option('bg_20')) { ?>g-line<?php } ?> sort" name="<?php echo zm_get_option('tool_s'); ?>">
	<div class="g-col">
		<div class="group-tool-box">
			...
			<?php $posts = get_posts( array( 'post_type' => 'any', 'orderby' => 'menu_order', 'meta_key' => 'tool_ico', 'numberposts' => '16') ); if($posts) : foreach( $posts as $post ) : setup_postdata( $post ); ?>
			<?php 
			...
			?>
			<div class="sx4">
				<div class="group-tool sup wow fadeInUp bk">
					<div class="group-tool-img"><div class="group-tool-img-top bgt"></div><?php zm_long_thumbnail(); ?></div>
					<div class="group-tool-pu bgt">
						...
						<?php if ( get_post_meta($post->ID, 'tool_button', true) ) { ?>
							<h3 class="group-tool-title wow fadeIn"><?php the_title(); ?></h3>
						<?php } else { ?>
							<a href="<?php the_permalink(); ?>" target="_blank" rel="external nofollow"><h3 class="group-tool-title wow fadeIn"><?php the_title(); ?></h3></a>
						<?php } ?>
